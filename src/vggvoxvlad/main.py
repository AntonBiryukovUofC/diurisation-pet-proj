...
parser.add_argument("--data_path", default="D:\\VoxCeleb\\vox1_dev_wav\\wav", type=str)
...
def main():
    ...
    trnlist, trnlb = toolkits.get_voxceleb2_datalist(
        args, path="./meta/voxlb1_train.txt"
    )
    vallist, vallb = toolkits.get_voxceleb2_datalist(args, path="./meta/voxlb1_val.txt")

    # construct the data generator.
    params = {
        "dim": (257, 250, 1),
        "mp_pooler": toolkits.set_mp(processes=10),
        "nfft": 512,
        "spec_len": 250,
        "win_length": 400,
        "hop_length": 160,
    ...
    network = model.vggvox_resnet2d_icassp(
        input_dim=params["dim"], num_class=params["n_classes"], mode="train", args=args
    )
    ...
    print(
        "==> gpu {} is, training {} images, classes: 0-{} "
    ...
    callbacks = [
        keras.callbacks.ModelCheckpoint(
            os.path.join(model_path, "weights-{epoch:02d}-{acc:.3f}.h5"),
            monitor="loss",
            mode="min",
            save_best_only=True,
        ),
...
def set_path(args):
    ...
    if args.aggregation_mode == "avg":
        exp_path = os.path.join(
            args.aggregation_mode + "_{}".format(args.loss),
            "{0}_{args.net}_bs{args.batch_size}_{args.optimizer}_"
            "lr{args.lr}_bdim{args.bottleneck_dim}_ohemlevel{args.ohem_level}".format(
                date, args=args
            ),
        )
        ...
        exp_path = os.path.join(
            ...
            "bdim{args.bottleneck_dim}_ohemlevel{args.ohem_level}".format(
                date, args=args
            ),
        )
