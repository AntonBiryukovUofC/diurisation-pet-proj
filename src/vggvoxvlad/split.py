...
def voxceleb1_split(path, network, split_seconds=3,shift_seconds = 1.5, n_top_candidates=3,
                    win_length=400, sr=16000, hop_length=160,
                    n_fft=512, spec_len=250, n_classes=1251, metafile_location = '../data/raw/vox1_meta.txt'):
    ...
    num_segments = int(np.floor( (df_audio['Time'].max()-split_seconds/2) / shift_seconds))
    ...
    for k in trange(num_segments):
        ...
        for i in range(n_top_candidates):
            df_out = df_out.append(pd.DataFrame({'Time_s': t,
