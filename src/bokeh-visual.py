...
df_small['amplitude'] = (df_small['amplitude'] - df_small['amplitude'].min()) / (
            df_small['amplitude'].max() - df_small['amplitude'].min()) - 0.5
...
for t in times_speakers:
    tmp = df_prob[df_prob.index <= t].groupby('Speaker').last().sort_values(
        'cumsum_prob').reset_index().tail(10)
...
p_prob = figure(x_range=source_prob.data['Speaker'], plot_width=int(2 * width / 3), plot_height=int(1.5 * height),
                tools=[],tooltips = tt)
