...
def train(model_path):
    ...
    optimizer = torch.optim.SGD(
        [{"params": embedder_net.parameters()}, {"params": ge2e_loss.parameters()}],
        lr=hp.train.lr,
    )
    ...
    for e in range(hp.train.epochs):
        ...
        for batch_id, mel_db_batch in enumerate(train_loader):
            ...
            perm = random.sample(
                range(0, hp.train.N * hp.train.M), hp.train.N * hp.train.M
            )
            unperm = list(perm)
        ...
        if (
            hp.train.checkpoint_dir is not None
            and (e + 1) % hp.train.checkpoint_interval == 0
        ):
            embedder_net.eval().cpu()
            ckpt_model_filename = (
                "ckpt_epoch_" + str(e + 1) + "_batch_id_" + str(batch_id + 1) + ".pth"
            )
    ...
    embedder_net.eval().cpu()
    save_model_filename = (
        "final_epoch_" + str(e + 1) + "_batch_id_" + str(batch_id + 1) + ".model"
    )
