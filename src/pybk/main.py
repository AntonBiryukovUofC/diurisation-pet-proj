...
def runDiarization(showName, config):
    ...
    if np.floor(
        (nSpeechFeatures - config.getint("KBM", "windowLength"))
    ...
    kbm, gmPool = trainKBM(
        data, config.getint("KBM", "windowLength"), windowRate, kbmSize
    )
    print("Selected", kbmSize, "gaussians from the pool")
    Vg = getVgMatrix(
        data, gmPool, kbm, config.getint("BINARY_KEY", "topGaussiansPerFrame")
    )
    ...
    print(
        "Number of clusters:\t",
        np.size(
            np.unique(finalClusteringTable[:, bestClusteringID.astype(int) - 1]), 0
        ),
    )
    if (
