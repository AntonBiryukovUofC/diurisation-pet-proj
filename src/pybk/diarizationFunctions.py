...
def readUEMfile(path, filename, ext, nFeatures, frameshift):
    ...
    if C:
        ...
        print(
            "UEM annotations file was empty. The complete file is considered evaluable."
        )
        ...
        if len(C[0]) > 1:
            if format == "LBL":
                ...
                for idx, x in enumerate(C):
                    ...
                    endTime = np.append(
                        endTime,
                        float(x.strip().split(" ")[3]) + float(x.strip().split(" ")[4]),
                    )
            elif format == "MDTM":
                for idx, x in enumerate(C):
                    initTime = np.append(initTime, float(x.strip().split(" ")[2]))
                    endTime = np.append(
                        endTime,
                        float(x.strip().split(" ")[2]) + float(x.strip().split(" ")[3]),
                    )
    else:
...
def getBestClustering(bestClusteringMetric, bkT, cvT, clusteringTable, n):
    ...
    if bestClusteringMetric == "cosine":
        ...
        distances = cdist(
            np.expand_dims(overallMean, axis=0), bkT, bestClusteringMetric
        )
...
def getSpectralClustering(
    ...
    thresholds = np.tile(
        percentile * np.nanmax(simMatrix, 1) / 100, (np.size(bkT, 0), 1)
    ).T
    ...
    simMatrix_5 = (
        simMatrix_4 / np.tile(simMatrix_4.max(axis=0), (np.size(simMatrix_4, 0), 1)).T
    )
    ...
    if format == "MDTM":
        for i in np.arange(np.size(seg, 0)):
            solution.append(
                showName
                + " 1 "
                + str(np.around(seg[i, 0], decimals=4))
                + " "
                + str(np.around(seg[i, 1], decimals=4))
                + " speaker NA unknown speaker"
                + str(seg[i, 2].astype(int))
            ) + "\n"
    elif format == "RTTM":
        for i in np.arange(np.size(seg, 0)):
            solution.append(
