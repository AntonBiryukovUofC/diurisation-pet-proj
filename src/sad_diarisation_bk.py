...
from src.pybk.diarizationFunctions import extractFeatures, readUEMfile, readSADfile, getSegmentTable, trainKBM, \
    getVgMatrix, \
    getSegmentBKs, performClustering, performClusteringLinkage, getBestClustering, getSpectralClustering, \
...
def run_diarization(showName, config):
    ...
    all_data = extractFeatures(config['PATH']['audio'] + showName + config['EXTENSION']['audio'],
                               config.getfloat('FEATURES', 'framelength'), config.getfloat('FEATURES', 'frameshift'),
                               config.getint('FEATURES', 'nfilters'), config.getint('FEATURES', 'ncoeff'))
    ...
    if os.path.isfile(config['PATH']['UEM'] + showName + config['EXTENSION']['UEM']):
        mask_uem = readUEMfile(config['PATH']['UEM'], showName, config['EXTENSION']['UEM'], n_features,
    ...
    del all_data
    segment_table = getSegmentTable(mask, speech_mapping, config.getint('SEGMENT', 'length'),
    ...
    if np.floor((n_speech_features - config.getint('KBM', 'windowLength')) / config.getint('KBM',
                                                                                           'minimumNumberOfInitialGaussians')) < config.getint(
        'KBM', 'maximumKBMWindowRate'):
        window_rate = int(np.floor((np.size(data, 0) - config.getint('KBM', 'windowLength')) / config.getint('KBM',
                                                                                                             'minimumNumberOfInitialGaussians')))
    else:
        window_rate = int(config.getint('KBM', 'maximumKBMWindowRate'))
    pool_size = np.floor((n_speech_features - config.getint('KBM', 'windowLength')) / window_rate)
    ...
    print('Training pool of', int(pool_size), 'gaussians with a rate of', int(window_rate), 'frames')
    kbm, gm_pool = trainKBM(data, config.getint('KBM', 'windowLength'), window_rate, kbm_size)
    print('Selected', kbm_size, 'gaussians from the pool')
    vg = getVgMatrix(data, gm_pool, kbm, config.getint('BINARY_KEY', 'topGaussiansPerFrame'))
    print('Computing binary keys for all segments... ')
    segment_bk_table, segment_cv_table = getSegmentBKs(segment_table, kbm_size, vg,
                                                       config.getfloat('BINARY_KEY', 'bitsPerSegmentFactor'),
                                                       speech_mapping)
    print('Performing initial clustering... ')
    initial_clustering = np.digitize(np.arange(numberOfSegments), np.arange(0, numberOfSegments,
                                                                            numberOfSegments / config.getint(
                                                                                'CLUSTERING', 'N_init')))
    print('done')
    print('Performing agglomerative clustering... ')
    if config.getint('CLUSTERING', 'linkage'):
        final_clustering_table, k = performClusteringLinkage(segment_bk_table, segment_cv_table,
                                                             config.getint('CLUSTERING', 'N_init'),
                                                             config['CLUSTERING']['linkageCriterion'],
                                                             config['CLUSTERING']['metric'])
    else:
        final_clustering_table, k = performClustering(speech_mapping, segment_table, segment_bk_table, segment_cv_table,
                                                      vg,
                                                      config.getfloat('BINARY_KEY', 'bitsPerSegmentFactor'), kbm_size,
                                                      config.getint('CLUSTERING', 'N_init'), initial_clustering,
                                                      config['CLUSTERING']['metric'])
    print('Selecting best clustering...')
    if config['CLUSTERING_SELECTION']['bestClusteringCriterion'] == 'elbow':
        best_clustering_id = getBestClustering(config['CLUSTERING_SELECTION']['metric_clusteringSelection'],
                                               segment_bk_table, segment_cv_table, final_clustering_table, k)
    elif config['CLUSTERING_SELECTION']['bestClusteringCriterion'] == 'spectral':
        best_clustering_id = getSpectralClustering(config['CLUSTERING_SELECTION']['metric_clusteringSelection'],
                                                   final_clustering_table, config.getint('CLUSTERING', 'N_init'),
                                                   segment_bk_table, segment_cv_table, k,
    ...
    print('Number of clusters:\t', np.size(np.unique(final_clustering_table[:, best_clustering_id.astype(int) - 1]), 0))
    if config.getint('RESEGMENTATION', 'resegmentation') and np.size(
            np.unique(final_clustering_table[:, best_clustering_id.astype(int) - 1]), 0) > 1:
        print('Performing GMM-ML resegmentation...')
        final_clustering_table_resegmentation, final_segment_table = performResegmentation(data, speech_mapping, mask,
                                                                                           final_clustering_table[:,
                                                                                           best_clustering_id.astype(
                                                                                               int) - 1],
                                                                                           segment_table,
                                                                                           config.getint(
                                                                                               'RESEGMENTATION',
    ...
    if config.getint('OUTPUT', 'returnAllPartialSolutions'):
        ...
        output_path_ind = config['PATH']['output'] + config['EXPERIMENT']['name'] + '/' + showName + '/'
        ...
        for i in np.arange(k):
            getSegmentationFile(config['OUTPUT']['format'], config.getfloat('FEATURES', 'frameshift'), segment_table,
                                final_clustering_table[:, i], showName,
                                showName + '_' + str(np.size(np.unique(final_clustering_table[:, i]), 0)) + '_spk',
